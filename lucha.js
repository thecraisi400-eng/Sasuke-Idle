...
function statLevels(){
  ...
  const out = {};
  STAT_DEFS.forEach(d=> out[d.out] = Math.max(0, Number(src[d.key].value) || 0));
  return out;
}
function enemyStatLevels(){
  const points = randInt(15,25);
  const levels = {};
  STAT_DEFS.forEach(d=> levels[d.out] = 0);
  for(let i=0;i<points;i++) levels[pick(STAT_DEFS).out] += 1;
  return levels;
}
function schedule(fn, ms){
  const id = setTimeout(()=>{ scheduledTimeouts = scheduledTimeouts.filter(t=>t!==id); fn(); }, ms);
...
function combatStats(levels){ return { maxHp:Math.max(1, Math.round(levels.hp)), atk:Math.max(1, levels.ataque), def:Math.max(0, levels.defensa), critChance:levels.critico, evasion:levels.evasion, speed:80+levels.velocidad*6 }; }
...
function fighterCardHTML(kind, name, tag, colors, accessory){
  ...
  return '<div class="lucha-fcard '+kind+'"><div class="lucha-fcard-glow" style="background:'+colors.head+'"></div><div class="lucha-fcard-sprite-wrap"><div class="lucha-character"><div class="lucha-char-shadow"></div>'+svg+'</div></div><div class="lucha-fcard-name">'+name+'</div><div class="lucha-fcard-tag">'+tag+'</div></div>';
}
function foundWindowHTML(enemyObj){
  return '<div class="lucha-found-window"><div class="lucha-search-window-header"><div class="lucha-search-dots"><span class="lucha-search-dot red"></span><span class="lucha-search-dot yellow"></span><span class="lucha-search-dot green"></span></div><div class="lucha-search-window-title">FICHA DE COMBATE</div><div style="width:15px"></div></div><div class="lucha-found-body"><div class="lucha-found-arena">'+fighterCardHTML('you','Tu Boxeador','Retador Local',PLAYER_COLORS,'none')+'<div class="lucha-vs-impact"><div class="lucha-vs-burst"></div><div class="lucha-vs-impact-text">VS</div></div>'+fighterCardHTML('cpu',enemyObj.name,enemyObj.rank,enemyObj.colors,enemyObj.accessory)+'</div><div class="lucha-search-status" id="lucha-found-status-text">¡Rival encontrado!</div><div class="lucha-search-progress-bg"><div class="lucha-search-progress-fill" id="lucha-found-bar" style="background:linear-gradient(90deg,#ffd54a,#e6ac00)"></div></div></div></div>';
}
function buildPanels(){
  luchaPanel.innerHTML = '<div class="lucha-ring-header"><div class="timer" id="lucha-timer-display">⏱ --</div></div><div class="lucha-ring-wrap" id="lucha-ring-wrap"><div class="lucha-ring-container"><div class="lucha-ring-floor"></div>'+['top','bottom','left','right'].map(side=>[1,2,3,4].map(i=>'<div class="lucha-rope-line lucha-rope-'+side+' r'+i+'"></div>').join('')).join('')+'<div class="lucha-corner-pad tl"></div><div class="lucha-corner-pad tr"></div><div class="lucha-corner-pad bl"></div><div class="lucha-corner-pad br"></div><div class="lucha-corner-post tl"></div><div class="lucha-corner-post tr"></div><div class="lucha-corner-post bl"></div><div class="lucha-corner-post br"></div><div class="lucha-mat" id="lucha-mat"></div></div><div class="lucha-overlay" id="lucha-overlay"><div class="lucha-radar-box"></div><div>Toca el ring para buscar rival</div></div></div>';
  luchaStatsPanel.innerHTML = ['you','cpu'].map(type=>'<div class="lucha-stats-col '+type+'"><div class="lucha-stats-header"><div class="lucha-stats-avatar">'+(type==='you'?'🥊':'🤖')+'</div><div><div class="lucha-stats-header-title">'+(type==='you'?'TÚ':'CPU')+'</div><div class="lucha-stats-header-sub">'+(type==='you'?'Tu boxeador':'Rival encontrado')+'</div></div></div><div class="lucha-stats-scroll" id="lucha-stats-'+type+'"></div></div>').join('');
  matEl=document.getElementById('lucha-mat'); ringWrap=document.getElementById('lucha-ring-wrap'); overlay=document.getElementById('lucha-overlay'); timerDisplay=document.getElementById('lucha-timer-display'); statsYouEl=document.getElementById('lucha-stats-you'); statsCpuEl=document.getElementById('lucha-stats-cpu');
...
function refreshFightStats(enemyLevels=null){ const levels=statLevels(); renderStatRows(statsYouEl, levels, 'you'); renderStatRows(statsCpuEl, enemyLevels, 'cpu'); }
...
function cleanupSearchTimers(){ if(searchTimeoutId) clearTimeout(searchTimeoutId); [searchTextInt,searchBlipInt,searchPingInt].forEach(id=>id&&clearInterval(id)); searchTimeoutId=searchTextInt=searchBlipInt=searchPingInt=null; }
...
function startCountdown(){ state=STATE.COUNTDOWN; const seq=['3','2','1','¡PELEA!']; let i=0; (function step(){ if(i>=seq.length){hideOverlay(); beginFight(); return;} const label=seq[i++]; showOverlay('<div class="'+(label==='¡PELEA!'?'lucha-fight-flash':'lucha-count-num')+'">'+label+'</div>', true); schedule(step,label==='¡PELEA!'?550:700); })(); }
...
function closeLuchaPanel(){ cleanupSearchTimers(); cleanupScheduledTimeouts(); cancelAnimationFrame(rafId); clearInterval(fightTimerInterval); if(matEl){ matEl.innerHTML=''; hideOverlay(); timerDisplay.textContent='⏱ --'; } player=null; enemy=null; state=STATE.IDLE; luchaPanel.classList.remove('show'); luchaStatsPanel.classList.remove('show'); }
