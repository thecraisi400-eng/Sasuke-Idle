...
function renderAttrsModal() {
  return window.renderAttrsModalContent(G);
}

function buyAttrUpgrade(i) {
  window.buyAttrUpgradeIndex(i, G, updateUI, openModal, showToast);
}
...
function doPrestige() {
  ...
  if (window.ATTR_STATE) {
    window.ATTR_STATE.pointsAvailable = 0;
    window.ATTR_STATE.upgrades.forEach(u => {
      u.level = 0;
      u.pointCost = u.id === 'carbon-edge' ? 1 : 0;
      if (u.id === 'carbon-edge') u.damageMultiplier = 1.30;
    });
  }
